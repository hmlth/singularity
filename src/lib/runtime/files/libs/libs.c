...
int _singularity_runtime_files_libs(void) {
    ...
    if ( ( includelibs_string = singularity_registry_get("CONTAINLIBS") ) != NULL ) {
        ...
        // Create the base lib directory inside the container
        if ( is_dir(libdir_contained) != 0 ) {
        ...
        for (idx = 0; idx < nb_supported_archs; idx++) {
            ...
            char *subdir_path = strjoin(joinpath("/.singularity.d/libs/", supported_archs[idx]), ":");
            singularity_message(DEBUG, "Examining libs subdir arch %d, %s, (%s, %s)\n", idx, supported_archs[idx], subdir, subdir_path);
            if ( is_dir(subdir) == 0 ) {
